...
function directoryTreeInternal (path, options, onEachFile, onEachDirectory, currentDepth) {
	...
	const recursionLimit = options && options.recursionDepth ? true : false;

	const item = { path, name };

	// Check to se eif we hit recursion depth and return if we have.
	if(recursionLimit && currentDepth == options.recursionDepth) {
		return item;
